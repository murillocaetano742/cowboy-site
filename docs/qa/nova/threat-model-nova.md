# Threat model — site COWBOY Energia (página nova + APIs)
...
| --- | --- | --- |
| Páginas estáticas (`index.html`, `cowboy-nova.html`, `cowboy-v3.html`, políticas) | `scripts/build-site.js` copia somente a allowlist para `dist/` | Superfície pública; nenhum dado de usuário persistido |
...
- **Gitleaks** `dir .`: 1 achado, regra `generic-api-key`, arquivo `.env.local` (não versionado). `git .`: 21 commits, 0 achados. Relatórios em `docs/qa/nova/gitleaks*.json` (contêm trechos do segredo: **não versionar**; estão cobertos por `.gitignore`? Não. Ver ação abaixo).
...
1. Não versionar `docs/qa/nova/gitleaks.json` (contém o trecho do segredo). Adicionado ao `.gitignore` nesta entrega.
