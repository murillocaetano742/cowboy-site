# COWBOY Energia — mapa da página e copy (versão nova, mobile first)

Data: 08/09/2026. Arquivo da página: `cowboy-nova.html` (rota `/nova` na Vercel, `/cowboy-nova` no servidor local). Estilos em `assets/css/cowboy-nova.css`, script em `assets/js/cowboy-nova.js`. Mundo visual baseado na versão publicada (preto, dourado em degradê, Oswald em caixa alta, cards com borda fina, botões dourados), executado mobile first. O modelo 3D foi retirado a pedido do proprietário e substituído por uma galeria de produto.
...
5. **Imagens.** Fotos escuras de produto já existentes (frasco único, kit de 2, kit de 4), macro do conta-gotas, cena da fórmula e entrega, em galeria deslizável. Sem 3D.
...
- **Faixa dourada:** `Composição aberta no rótulo | Preço fechado antes de pagar`. Marca `COWBOY Energia`; links (desktop): `Como funciona` · `Composição` · `Compromisso` · `Relatos` · `Perguntas`.
...
- **Título:** `ENERGIA EM GOTAS. RÓTULO ABERTO. PREÇO FECHADO.` (linha do meio em dourado)
- **Texto:** `Seis componentes em uma única solução líquida, medida no conta-gotas. Aqui você vê a composição por porção, o modo de uso e quem fabrica, antes de decidir.`
- **Lista com check:** os quatro fatos verificáveis (componentes, frasco, CNPJ, preço total).
- **Botão:** `Como funciona` → capítulo 1. Nota: `Os kits e a compra ficam no fim da página, de propósito.`
- **Números:** `6 componentes por porção` · `30 mL por frasco` · `12 gotas por porção` · `30 porções declaradas`.
- **Base:** volume, porções, componentes e fabricante transcritos do rótulo (`product-facts.json`).

### Bloco "Você se identifica?" (antes dos capítulos)

- **Função:** espelhar o perfil do comprador sem prometer efeito. Substitui o bloco de "problemas" da versão publicada, que fazia alegações de saúde.
- **Título:** `COWBOY É PARA VOCÊ SE…`
- **Quatro cartões:** `Você quer saber o que toma` · `Prefere gotas a cápsulas` · `Quer preço fechado` · `Exige origem identificada` (textos no HTML).

### Capítulo 2 — O frasco (galeria)

- **Função:** familiaridade com o objeto que vai chegar. Galeria deslizável com cinco imagens: frasco, conta-gotas, cena da fórmula, kit de 2, kit de 4.
- **Título:** `O FRASCO QUE CHEGA NA SUA CASA`
- **Aviso:** `Imagens ilustrativas produzidas a partir das fotos do produto. O rótulo real é a referência.`

### Capítulo 1 — Por que gotas (mecanismo COWBOY)
...
- **Linhas prontas, só após confirmação da operação (não publicadas):** `Lote e validade conferidos antes do envio.` · `Embalagem discreta, sem identificação do conteúdo.` · `Código de rastreio enviado após a postagem.` · `PIX, boleto ou cartão` (confirmar no painel Cartpanda) · Número de processo do rótulo, somente após consulta da situação atual na Anvisa.
...
Perguntas de `copy-v2.md` mais uma nova: `Por que os botões de compra ficam só no fim?` (`Porque queremos que você conheça o produto inteiro antes de decidir. Composição, modo de uso e compromisso vêm primeiro.`).
